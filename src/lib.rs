...
pub fn vector_from_hyperspace<T: Scalar + Zero + Float, DimX: Dim + DimName, DimY: Dim + DimName>(
    x_dual: &VectorN<DualN<T, DimY>, DimX>,
) -> VectorN<T, DimX>
where
    DefaultAllocator: Allocator<T, DimX> + Allocator<DualN<T, DimY>, DimX> + Allocator<T, DimY>,
