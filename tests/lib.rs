...
fn multivariate() {
    ...
    let y: DualN<f64, Const<3>> = Hyperdual::from_slice(&[5.0, 0.0, 1.0]);
...
fn state_gradient() {
    ...
    type StateVectorDualType = SVector<DualN<f64, Const<7>>, 6>;
    ...
    fn eom(_t: f64, state: &StateVectorDualType) -> StateVectorDualType {
        ...
        let body_acceleration = radius * (Hyperdual::<f64, Const<7>>::from_real(-398_600.441_5) / rmag.powi(3));
...
fn state_partials() {
    ...
    type StateVectorDualType = SVector<Hyperdual<f64, Const<7>>, 6>;
    type OutputVectorDualType = SVector<Hyperdual<f64, Const<7>>, 2>;
...
fn test_hyperspace_from_vector() {
    ...
    let hyperstate: SVector<DualN<f64, Const<7>>, 6> = hyperspace_from_vector(&vec);
...
fn test_vector_from_hyperspace() {
    ...
    let dual_vec = Vector6::new(
        DualN::<f64, Const<7>>::from_real(4_354.653_483_456_944),
        DualN::<f64, Const<7>>::from_real(18_090.191_366_880_514),
        DualN::<f64, Const<7>>::from_real(2_901.658_181_581_637),
        DualN::<f64, Const<7>>::from_real(-3.742_815_992_334_434_4),
        DualN::<f64, Const<7>>::from_real(0.901_480_766_308_994_1),
        DualN::<f64, Const<7>>::from_real(1.644_034_610_527_063_8),
    );
...
#[test]
fn test_hypot() {
    let x: Hyperdual<f64, Const<3>> = Hyperdual::from_slice(&[3.0, 1.0, 0.0]);
    let y: Hyperdual<f64, Const<3>> = Hyperdual::from_slice(&[4.0, 0.0, 1.0]);
...
fn test_div() {
    ...
    let y: Hyperdual<f64, Const<3>> = Hyperdual::from_slice(&[4.0, 0.0, 1.0]);
    let rslt = x / y;
    let expt: Hyperdual<f64, Const<3>> = Hyperdual::from_slice(&[3.0 / 4.0, 1.0 / 4.0, -3.0 / 16.0]);
...
#[test]
fn test_powf() {
    let x: Hyperdual<f64, Const<2>> = Hyperdual::from_slice(&[3.0, 1.0]);
    let y: Hyperdual<f64, Const<2>> = Hyperdual::from_slice(&[2.3, 0.0]);
    let rslt = x.powf(y);
    let expt: Hyperdual<f64, Const<2>> = Hyperdual::from_slice(&[12.513502532843182, 9.593685275179771]);
...
fn test_trig_atan2() {
    ...
    >>>
        ...
        */
    let x: Hyperdual<f64, Const<3>> = Hyperdual::from_slice(&[3.0, 0.0, 1.0]);
    let y: Hyperdual<f64, Const<3>> = Hyperdual::from_slice(&[2.0, 1.0, 0.0]);
    let rslt = y.atan2(x);
    let rslt_atan = (y / x).atan();
    let expt: Hyperdual<f64, Const<3>> = Hyperdual::from_slice(&[0.5880026035475675, 0.23076923076923075, -0.15384615384615383]);
...
fn test_trig_acos() {
    ...
    >>>
        */
    let x: Hyperdual<f64, Const<2>> = Hyperdual::from_slice(&[0.59, 1.0]);
    let rslt = x.acos();
    let expt: Hyperdual<f64, Const<2>> = Hyperdual::from_slice(&[0.9397374860168752, -1.238538495133269]);
...
fn test_trig_asin() {
    ...
    let x: Hyperdual<f64, Const<2>> = Hyperdual::from_slice(&[0.59, 1.0]);
    let rslt = x.asin();
    let expt: Hyperdual<f64, Const<2>> = Hyperdual::from_slice(&[0.6310588407780213, 1.238538495133269]);
